"""GitHub App Webhook Ingestion Endpoint with HMAC Verification & PR Orchestration."""
...
async def _handle_installation(session: AsyncSession, payload: dict[str, Any]) -> dict[str, Any]:
    """Handles GitHub App installation events, linking installation ID to matching Organization."""
...
async def _get_or_create_repo(session: AsyncSession, repo_data: dict[str, Any], clone_url: str, full_name: str) -> Repository:
    """Retrieves or registers repository record."""
    ...
    if not repo:
        repo = Repository(
            ...
            url=clone_url or f"https://github.com/{full_name}",
...
async def _handle_pull_request(session: AsyncSession, payload: dict[str, Any]) -> dict[str, Any]:
    """Executes diff-targeted AST analysis and automated review commenting on PR events."""
    action = payload.get("action")
    ...
    repo_record = await _get_or_create_repo(session, repo_data, clone_url, repo_full_name)
    ...
    if repo_record.organization_id:
        ...
        if org:
            ...
            if not is_permitted:
                return {"status": "rejected", "reason": quota_warning, "pr_number": pr_number}
    ...
    comment_res = await PRCommentService.execute_pr_review_actions(analysis_result, client=github_pr_client)
...
async def receive_github_webhook(
    ...
    if x_github_event == "installation":
        return await _handle_installation(session, payload)
